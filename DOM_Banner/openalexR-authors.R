...
get_works_from_authorid_by_year <- function(dept_code, author_id, year) {
  ...
  author_works <- tryCatch({
    oa_fetch(
      entity = "works",
      author.id = author_id, 
      #identifier = author_id, //why this not working
...
# 4. Kimberly Chapman (correct: ) Reason: There are two Kimberly: exactly the same name at UArizona. https://openalex.org/authors/A5078704068 : Steward Observatory. The other at UA Libraries
# 5. Daniel Martin:  Reason: same name. Note: linked to wrong institution
# 6. Yan Han  (correct: https://orcid.org/0000-0001-9518-2684). Reason: linked to wrong same-name person (which has a wrong affiliation https://openalex.org/A5056855636)
...
UAresult1$affiliations_other 

author_name <- "Fernando Rios"
affiliation_name <- "University of Arizona"
author_from_names <- oa_fetch(entity = "author", search = author_name)
UAresult1 <- search_author(author_name, affiliation_name)
UAresult1$affiliations_other  # [5] "https://openalex.org/I138006243"

author_name <- "Heather Froehlich"
affiliation_name <- "University of Arizona"
author_from_names <- oa_fetch(entity = "author", search = author_name)
# affiliated with Penn State https://openalex.org/A5009648082
UAresult1 <- search_author(author_name, affiliation_name)
...
# 2. Elizabeth Miller (no publications). reason: her affiliation is linked to UArizona. 

### Correct: all the rest

### Special handling:
# 1. Stacey Erdman (associated with )
# 2. Dana Lema ?
